...
.SYNOPSIS
    Creates a complete inventory of a Nutanix configuration using Microsoft Word.
.Nutanix Documentation Script
    NAME: NetScaler_Script_v2_0.ps1
	VERSION NetScaler Script: 2.0
	VERSION Script Template: 17072014
	AUTHOR Nutanix script: Kees Baggerman
    AUTHOR Script template: Carl Webster, Michael B. Smith, Iain Brighton, Jeff Wouters
	LASTEDIT: July 21, 2014  
.Release Notes version 1
    Version 1.0 supports the following Nutanix functionality:
	Nutanix System Information
	Alerts
...
.EXAMPLE
	PS C:\PSScript > .\c.ps1 -PDF -AddDateTime
...
Param(
    ...
    [string]$nxUser="nxUser"

    [parameter(
    Position = 8, 
    Mandatory=$False )
    ] 
    [string]$nxPassword="nxPassword"
...
# Add the path to the modules folder in PSModulePath variable to inform
# powershell to look in this folder for all the DLLs.
$env:PSModulePath = $env:PSModulePath + ";" + $dllPath

# Get the names of all the DLLs in an array
$dllArr = Get-ChildItem -Path $dllPath *.dll -recurse

Write-Host "Importing the modules..."

Import-Module Common

# Loop to import each of the DLLs.
for ($i = 0; $i -lt $dllArr.length; $i++)
{
    $currFile = $dllArr[$i]

    # Import every module except Common.dll since Common.dll is already imported
    if ([string]$currFile -ne "Common.dll")
    {
        Import-Module $currFile -WarningAction silentlyContinue `
            -Prefix $nounPrefix
    }
}

Write-Host "Finished importing the modules."
...
$oldLogs = $logDir + "\log_old.txt"
if (Test-Path $oldLogs)
{
    Remove-Item $oldLogs
}

$currLogs = $logDir + "\log.txt"
if (Test-Path $currLogs)
{
    Rename-Item $currLogs log_old.txt
}

New-Item $currLogs -type file


#endregion Nutanix Load CmdLets
...
$Params = $null    ## IB - In theory this isn't needed as you're reassigning the variable in the next line
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
   
    ## IB - Add our array of hashtables to the -Hashtable parameter
    Hashtable = $rackableUnits;
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "ID","Model","Location","Serial","Positions";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
$RemoteSetting = Get-NTNXRemoteSupportSetting;
$Params = $null    ## IB - In theory this isn't needed as you're reassigning the variable in the next line
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    Hashtable = @( ## IB - The -Hashtable parameter needs an array of hashtables. Note the @( and not @{
        @{         ## IB - This is a hash table. As it's the first, it'll be the first row in the table
            Enabled = $RemoteSetting.enabled;
            Duration = $RemoteSetting.duration;
        })

    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Enabled","Duration";
...
foreach ($hostconfig in Get-NTNXHost) {
    ...
    $hostconfigs += @{
        ...
        "Maintenance Mode" = $hostconfig.inMaintenanceMode;
        "MetaDataStatus" = $hostconfig.metadataStoreStatusMessage;
        ...
        "Memory in Bytes" = $hostconfig.memoryCapacityInBytes;
        "Hypervisor" = $hostconfig.hypervisorFullName;
        "Boot time in Seconds" = $hostconfig.bootTimeInUsecs;
    };
} # end foreach
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Name","Serial","Block Serial","Maintenance Mode","MetaDataStatus";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
WriteWordLine 3 0 "Nutanix Miscalenious Configuration"
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Name","Memory In Bytes","Hypervisor","Boot time in seconds";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $hostalerts = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($hostalert in Get-NTNXHostAlert) {
    ## IB - Add each enumerated object to the array
    $hostalert += @{
        "Severity" = $hostalert.severity;
        "Type" = $hostalert.AlertType;
        "Message" = $hostalert.message;
        "Not Resolved" = $hostalert.resolved;
    };
} # end foreach
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - Add our array of hashtables to the -Hashtable parameter
    Hashtable = $hostalerts;
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Severity","Type","Message","Not Resolved";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
WriteWordLine 0 0 "Note: If this (sub) chapter is empty there are no unresolved alerts"
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $hostevents = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($hostevent in Get-NTNXHostEvent) {
    ## IB - Add each enumerated object to the array
    $hostevent += @{
        "Severity" = $hostevent.severity;
        "Type" = $hostevent.AlertType;
        "Message" = $hostevent.message;
        "Not Resolved" = $hostevent.resolved;
    };
} # end foreach
...
WriteWordLine 0 0 "Note: If this (sub) chapter is empty there are no unresolved events"

WriteWordLine 0 0 " "

#endregion Nutanix Host Configuration

#region Nutanix Host Alerts
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $HWAlerts = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($HWAlert in Get-NTNXHardwareAlert) {
    ## IB - Add each enumerated object to the array
    $HWAlerts += @{
        "Severity" = $HWalert.severity;
        "Type" = $HWalert.AlertType;
        "Message" = $Hwalert.message;
        "Resolved" = $HWalert.Resolved;
    };
} # end foreach
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Severity","Type","Message","Resolved";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $CritAlerts = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($CritAlert in get-NTNXAlert) {
    ## IB - Add each enumerated object to the array
    $CritAlerts += @{
        "Severity" = $CritAlert.severity;
        "Type" = $CritAlert.AlertType;
        "Message" = $CritAlert.message;
    };
} # end foreach
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $ClAlerts = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($ClAlert in Get-NTNXMultiClusterAlert) {
    ## IB - Add each enumerated object to the array
    $ClAlerts += @{
        "Uuid" = $Clalert.alertTypeUuid;
        "Type" = $Clalert.AlertType;
        "Message" = $Clalert.message;
    };
} # end foreach
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $ClEvents = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($ClEvent in Get-NTNXMultiClusterEvent) {
    ## IB - Add each enumerated object to the array
    $ClEvents += @{
        "Uuid" = $ClEvent.alertTypeUuid;
        "Type" = $ClEvent.AlertType;
        "Message" = $ClEvent.message;
    };
} # end foreach
...
WriteWordLine 1 0 "Container Information"
...
foreach ($Container in Get-NTNXContainer) {
    ...
    $Containers += @{
        ...
        "Explicit Reserved" = $Container.totalExplicitReservedCapacity;
        "Implicit Reserved" = $Container.totalImplicitReservedCapacity;
        "Logical Size" = $Container.logicalSize;
        "Physical Size" = $Container.physicalSize;
        "RF" = $Container.replicationFactor;
        "OpLog RF" = $Container.oplogReplicationFactor;
        "Used Size" = $Container.usedPhysicalSize;
        "Provisioned Capacity" = $Container.provisionedCapacity;
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "ID","Name","Pool ID","Marked for removal","Max Capacity","Explicit Reserved","Implicit Reserved","Logical Size","Physical Size","Used Size";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns   
    Format = -235; ## IB - Word constant for Light Grid Accent 5 (could use -207 for Accent 3 (grey))
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "ID","Name","RF","OpLog RF","Provisioned Capacity","Fingerprint on Write","On Disk Dedup","Compression enabled";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
foreach ($DataStore in Get-NTNXNfsDatastore) {
    ...
    $DataStores += @{
        ...
        "Free Space" = $DataStore.freeSpace;
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Name","Container","Host ID","Host IP","Capacity","Free Space";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns    
    Format = -235; ## IB - Word constant for Light Grid Accent 5 (could use -207 for Accent 3 (grey))
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "ID","CVM ID","Mount Path","Disk Size","Hostname","CVM IP","Tier Name","Pool IP","Removal";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $DiskAlerts = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($DiskAlert in Get-NTNXDiskAlert) {
    ## IB - Add each enumerated object to the array
    $DisAlerts += @{
        "Type" = $DiskAlert.AlertType;
        "Message" = $DiskAlert.message;
    };
} # end foreach
...
WriteWordLine 0 0 "Note: If this (sub) chapter is empty there are no critical disk alerts"

WriteWordLine 3 0 "Nutanix Disk Events"

 
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $DiskEvents = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($DiskEvent in Get-NTNXMultiClusterEvent) {
    ## IB - Add each enumerated object to the array
    $DiskEvents += @{
        "Type" = $DiskEvent.AlertType;
...
#region Nutanix Events

$Chapter++
Write-Verbose "$(Get-Date): Chapter $Chapter/$Nutanix Events"
WriteWordLine 1 0 "Nutanix Events"
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $CritEvents = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($CritEvent in get-NTNXEvent) {
    ## IB - Add each enumerated object to the array
    $CritEvents += @{
        "Severity" = $CritEvent.severity;
        "CVM" = $CritEvent.AlertType;
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - Add our array of hashtables to the -Hashtable parameter
    Hashtable = $CritEvents;
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Severity","CVM","Message";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
#endregion Nutanix Events Configuration
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Container Name","Pool Name","Shared","NFS File","Immutable","Snapshot","Dedup","NFS File Name";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
foreach ($StoragePool in Get-NTNXStoragePool) {
    ...
    $StoragePools += @{
        ...
        "Used Capacity" = $StoragePool.usedCapacity;
        "Reserved Capacity" = $StoragePool.reservedCapacity;
        "Free Unreserved Capacity" = $StoragePool.freeUnreservedCapacity;
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "ID","Name","Capacity","Used Capacity","Reserved Capacity","Free UnReserved Capacity","ILM Threshold";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $SPEvents = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($SPEvent in Get-NTNXStoragePoolEvent) {
    ## IB - Add each enumerated object to the array
    $SPEvents += @{
        "Alert Type" = $SPEvent.AlertType;
        "Message" = $SPEvent.message;
    };
} # end foreach
...
foreach ($PSUnProtVM in Get-NTNXUnprotectedVm) {
    ...
    $PSUnProtVMs += @{
        ...
        "Memory" = $PSUnProtVM.memoryCapacityInBytes;
        "Res. Memory" = $PSUnProtVM.memoryReservedCapacityInBytes;
        "vCPUs" = $PSUnProtVM.numVCpus;
        "Res. Hz" = $PSUnProtVM.cpuReservedinHz;
        "NICs" = $PSUnProtVM.numNetworkAdapters;
        "CVM" = $PSUnProtVM.isControllerVm;
        "vDisk" = $PSUnProtVM.nutanixVirtualDisks;
        "Disk Capacity" = $PSUnProtVM.diskCapacityInBytes;
    };
} # end foreach
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Name","OS","Memory","Res. Memory","vCPUs","Res. HZ","NICs","CVM","Disk Capacity";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
foreach ($PSUnProtVM in Get-NTNXUnprotectedVm) {
    ...
    $PSUnProtVMs += @{
        "Name" = $PSUnProtVM.vmName;
        "vDisk" = $PSUnProtVM.nutanixVirtualDisks;
        "vDisk File Path" = $PSUnProtVM.vdiskFilePaths;
    };
} # end foreach
...
$Params = @{       ## IB - This hash table is used for splatting, i.e. the -Hashtable, -Columns, -Format and -AutoFit parameters
    ...
    ## IB - We need the -Columns parameter to specify the correct order (hash tables are unordered)
    ## IB - You don't need to quote here, e.g. 'Columns = ID,Model,Location,Serial,Positions,Nodes' would also work
    Columns = "Name","vDisk","vDisk File Path";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $SPAlerts = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($SPAlert in Get-NTNXProtectionDomainAlert) {
    ## IB - Add each enumerated object to the array
    $SPAlerts += @{
        "Alert Type" = $SPAlert.alertTypeUuid;
        "Message" = $SPAlert.message;
    };
} # end foreach
...
## IB - Create a typed array of Hashtables
[System.Collections.Hashtable[]] $PDEvents = @();
 
## IB - Loop through each returned rackable unit/object
foreach ($PDEvent in Get-NTNXProtectionDomainEvent) {
    ## IB - Add each enumerated object to the array
    $PDEvents += @{
