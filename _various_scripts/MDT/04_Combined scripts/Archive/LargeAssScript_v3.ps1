...
Param(
    ...
    #[Parameter(Mandatory = $true)]
    # [Alias('ResourceName')] [string]  $VMNetwork,                                            # Name of the VLAN
    [Parameter(Mandatory = $true)]
...
Try {
    ...
    Start-Sleep 10
        ...
        if ($Null -ne $Containerid) {
            ...
            If (!(Test-Path MDT:)) { New-PSDrive -Name MDT -Root $deploymentShare -PSProvider Microsoft.BDD.PSSNAPIN\MDTPROVIDER } -ErrorAction SilentlyContinue
