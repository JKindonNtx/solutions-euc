...
Param(
    ...
    [Parameter(Mandatory = $true)]
    [Alias('TaskSequence')] [string] $TaskSequence,
    [Parameter(Mandatory = $true)]
    ...
    Start-Sleep 10
        ...
        if ($Null -ne $Containerid) {
            ...
            if ($Null -ne $ISO) {
                ...
                New-MDTComputer -assettag $Name -macAddress $VMNIC.MacAddress.ToUpper() -settings @{SkipWizard="YES"; TaskSequenceID="$TaskSequence"; OSDComputerName="$Name"; SkipComputerName="YES"; SkipTaskSequence="YES" }
