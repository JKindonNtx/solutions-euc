...
In the following section, we cover the design decisions and rationale for Citrix DaaS deployments on Nutanix.

_Table: Platform Design Decisions_

| Design Area | Item | Detail | Rationale |
| --- | --- | --- | --- |
| General | Software versions | Citrix DaaS; Citrix Provisioning 2402; AOS 6.5.5.1 | – |
| General | Minimum size | 3 Nutanix nodes running AHV | Minimum size requirement |
| General | Scale approach | Incremental modular scale | (tens of servers) to massive scale (hundreds of servers) |
...
| Nutanix AHV | Datastores | 1 Nutanix storage datastore per pod (Citrix DaaS VMs, VM clones, and so on) | Nutanix handles I/O distribution and localization; n-Controller model |
...
| Citrix DaaS | Cloud Connectors | Minimum: 2 (n + 1); Scale: 1 per additional pod | High availability for Cloud Connectors |
| Citrix DaaS | Users per Resource Location | Up to 10,000 VDAs with up to 25,000 total sessions | Citrix DaaS limits |
| Citrix DaaS | Load balancing | Built into Cloud Connectors | Ensures availability of Cloud Connectors; balances load between Cloud Connectors |
| Citrix DaaS | Virtual hardware specs | vCPU: 4; Memory: 4+ GB (local host cache); Disk: 60 GB vDisk | Standard sizing practice |
| Citrix DaaS | Desktop management | Nutanix AHV plug-in desktop management | For deployment and management |
