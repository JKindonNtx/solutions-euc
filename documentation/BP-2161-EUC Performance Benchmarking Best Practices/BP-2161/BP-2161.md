...
<!--JK: Replaced with new text - added context on who is defining best practices - its subjective :)
...
<!--JK: Replaced this section the below simplified one
...
<!--JK: This is duplicated information from above, I've blanked it here, and taken the education bit into the above intro section
...
<!--JK: Had a crack at rewording the below and pulling to the above - lots of words, not quite sure if mine is any better - feel like we are crossing concepts of benchmarking with user experience specifics - not sure how much we want to do that "User experience measurements is a benefit of benchmarking may be enough-->
...
<!--JK: used less words, not sure if it's any better
...
<!--JK: I would suggest changing removing the word Irrelevant and giving it a clear metric - even if that metric is "less than 100%" or whatever we are trying to say-->
...
<!--JK: What do we mean by current? Don't we mean Max?-->
...
<!--JK: This is very interesting. Do we define this, do customers define this, what are we trying to get at here - the importance of benchmarking, or what customers should aim to get too? I feel like this opens up a can of worms. I think we should remove the "good result bit" or specify that is what we look for?-->
<!--JK: Login time or Logon time? Either works, let's find and replace once decided-->
...
<!--JK: Altered the above points, this might impact your image, your choice :)-->
...
<!--JK: I have removed these points, I don't think they are relevant. You cover them in the first point - which I have expanded.
...
You can also reboot the client virtual machines used to start the user sessions (launchers) before each test. However, in some cases, this can result in slower login times for the first user starting a session from that client. <!--JK: Say what? What are we trying to say here? I think it needs a reword?:::: This will affect the average login times and is something to be mindful of.-->
...
<!--JK: halting here to make sure my review feedback is on the right path.-->
