...
Nutanix Flow Network Security is an application-centric security offering for enterprise applications running on Nutanix AHV, such as Citrix Virtual Apps and Desktops. <!--JK: @david-brett I think this previous sentence is superfluous - it's covered in the section before? --> In a typical CVAD environment, you can protect two distinct types of entities with Flow: 
...
<!--JK @david-brett I took the below table, and tried my hand at re-wording it to the above table. I found it a touch confusing following the logic and our language changes from "we did" to "you should" so I tried to align it to your entry statement of "we will be using 2 built-in categories... If you don't like it, we can kill it off -->

<!-- 
| **Category** | **Description** | 
| --- | --- |
| AppType | Add values for application types in your environment to the ***AppType*** category (In this example, "*Citrix Infrastructure*" and "*Citrix Workers*" are two application types). Use values to divide the application into logical types when configuring a **security policy**. |
| AppTier | Associate the VMs in your AppType category with the appropriate built-in **AppTier**. Examples include "*Citrix StoreFront*" and "*Citrix Licensing*". | 
-->
...
<!--JK: @david-brett our language changes again below in the table to a "you should" from a "we will" in the above sentence - worth choosing a path and aligning to it? Either a we did (example based) or you should (with guidance) -->
...
- Citrix Workers (Virtual Desktop Agents) <!-- JK: @david-brett I know you are going to hate me for this, so ignore if needed, but what I would love to see here would be a micro-seg example between HR and Finance VDA's :) -->
<!--JK: @david-brett Hello Provisioning Services, I miss you -->
...
As you can see we have broken the categories down into 2 main **AppTypes** and their associated **AppTier**: <!--JK: @david-brett I tried to simplify the below - see what you think-->
...
- AppType: Citrix Workers
  - AppTier: Citrix Desktop OS <!--JK: @david-brett This is what I was missing above, but if possible, it would hold more weight if we used departmental based examples? Just a thought-->
...
<!-- JK: @david-brett simplified wording below-->
...
First, let's take a look at the overall goal we are trying to achieve. <!--JK @david-brett This feels out of place -> you have a nice image below which really sets the entire scene for everything else, maybe consider restructuring to lead with this, and then keep linking back to to it?-->
...
We can break the AppTier's into the following logical groups. <!--JK: @david-brett didn't we already do this above? I feel this might be duplicated but we have Active Directory and Prism included here which we didn't in the above (probably because this list is based off the diagram) -->
...
<!--JK @david-brett was experimenting to see if a single table with multiple services might be easier on the eyes - not sure if it worked or not - see what you think
...
<!--JK: @david-brett hid this table for now if we stick with the list format below
| **Policy Type** | **Purpose** | 
| --- | --- | 
...
<!--JK: @david-brett we are talking about application policies and then instantly into isolation policies - should these be defined as different entities? -->
...
<!---JK: @david-brett simplified the below-->
