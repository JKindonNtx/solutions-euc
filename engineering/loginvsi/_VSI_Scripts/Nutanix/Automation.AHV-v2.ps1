...
function Capture-HostData {
    ...
    $uri = "https://10.56.68.130:9440/PrismGateway/services/rest/v2.0/hosts?filter=name==DRMAMDKB-4"
...
[ServerCertificateValidationCallback]::Ignore()
    write-host "use Invoke-RestMethod to get server uuid"    
    $hosts = Invoke-RestMethod -Method GET -Uri $uri -Header $header
    #$server = $hosts | Where-Object {$_.name -eq 'DRMAMDKB-4'}
    $server = $hosts[1]
    $hosts.uuid
    write-host "server uuid is:" $hosts.uuid
