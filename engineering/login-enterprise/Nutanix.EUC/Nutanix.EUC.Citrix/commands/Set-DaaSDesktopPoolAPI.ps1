function Set-DaaSDesktopPoolAPI {
    ...
    param(
        ...
        [Parameter(ValuefromPipelineByPropertyName = $true, mandatory = $false)][string]$DomainAdminCredential,
    ...
    #----------------------------------------------------------------------------------------------------------------------------
    $Method = "Get"
    $RequestUri = "https://$CloudUrl/cvad/manage/DeliveryGroups/"
    #----------------------------------------------------------------------------------------------------------------------------
    #----------------------------------------------------------------------------------------------------------------------------
    ...
    if (-not [string]::IsNullOrEmpty($delivery_group_exists)) {
        ...
        if ($CloneType -eq "MCS") {
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "Get"
            $RequestUri = "https://$CloudUrl/cvad/manage/MachineCatalogs/"
            #----------------------------------------------------------------------------------------------------------------------------
            try {
    ...
    if ($CreatePool -eq $true -or $Force) {
        ...
        if (-not [string]::IsNullOrEmpty($delivery_group_exists)) {
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "Delete"
            $RequestUri = "https://$CloudUrl/cvad/manage/DeliveryGroups/$($DesktopPoolName)?async=true"
            #----------------------------------------------------------------------------------------------------------------------------
            #----------------------------------------------------------------------------------------------------------------------------
            # Check that creds aren't expired before proceeding
            #----------------------------------------------------------------------------------------------------------------------------
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "Get"
            $RequestUri = "https://$CloudUrl/cvad/manage/Jobs/"
            #----------------------------------------------------------------------------------------------------------------------------
            try {
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "Get"
            $RequestUri = "https://$CloudUrl/cvad/manage/Jobs/$($job.id)"
            #----------------------------------------------------------------------------------------------------------------------------
            try {
        ...
        if (-not ([string]::IsNullOrEmpty($catalog_exists))) {
            ...
            #----------------------------------------------------------------------------------------------------------------------------
            $Method = "Delete"
            $RequestUri = "https://$CloudUrl/cvad/manage/MachineCatalogs/$($DesktopPoolName)?deleteVm=true&purgeDBOnly=false&deleteAccount=Delete&async=true" #this creates a job we need to go find with type DeleteMachineCatalog
            #----------------------------------------------------------------------------------------------------------------------------
            #----------------------------------------------------------------------------------------------------------------------------
            # Check that creds aren't expired before proceeding
            #----------------------------------------------------------------------------------------------------------------------------
        ...
        #JK - Need to add a validation on the hosting connection network here
        ...
        #----------------------------------------------------------------------------------------------------------------------------
        $Method = "Post"
        $RequestUri = "https://$CloudUrl/cvad/manage/MachineCatalogs?async=true"
        ...
        try {
            Write-Log -Message "Attempting to create Catalog $($DesktopPoolName) in Citrix DaaS" -Level Info
        ...
        #----------------------------------------------------------------------------------------------------------------------------
        $Method = "Get"
        $RequestUri = "https://$CloudUrl/cvad/manage/Jobs/$($target_job.Id)"
        #----------------------------------------------------------------------------------------------------------------------------
        try {
        ...
        # Set API call detail
        $Method = "Post"
        $RequestUri = "https://$CloudUrl/cvad/manage/DeliveryGroups/?async=false"
        $ContentType = "application/json"
        $PayloadContent = @{
