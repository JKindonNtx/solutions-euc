function Invoke-PublicApiMethodRedfish {
    param
    (
        $IPMI_ip,
        $Path,
    ...
    $header = @{
        Authorization = "Basic " + [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($($VSI_Target_IPMI_admin) + ":" + $($VSI_Target_IPMI_Password)))
    }
    ...
    if ($PSEdition -eq "Core") {
        ...
        while ($done -eq $false) {
            $count++
            try {
                $URL = "https://$($IPMI_ip)/redfish/v1/$Path"
                if ($null -ne $Body) {
                    if ($null -ne $OutFile) {
