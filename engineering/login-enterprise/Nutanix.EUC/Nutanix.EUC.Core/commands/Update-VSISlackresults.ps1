function Update-VSISlackresults {
    ...
    .EXAMPLE
    PS> Update-VSISlackesults -TestName "Testname" -Path $Path
...
#>
    ...
    Param (
        ...
        $RestError = $null
        Try {
            Invoke-RestMethod -uri $($config.TestInfra.Slack) -Method Post -body $body -ContentType 'application/json' -ErrorAction Stop | Out-Null
        } Catch {
            $RestError = $_
        }
