function Enable-VSIHVDesktopPool {
    ...
    if (!($null -eq (Get-HVPool -PoolName $Name))) {
        Write-Log -Message "Pool $($Name) Found" -Level Info
        Write-Log -Message "Getting VMs in Pool $($Name)" -Level Info
        ...
        Write-Log -Message "Disable Pool $($Name)" -Level Info
    ...
    if ($RDSH) {
        Write-Log -Message "Enabling provisioning of farm F_$Name with $VMAmount VMs" -Level Info
        ...
        if ($AllMachinesUpFront) {
            if ($Increment -gt 0) {
                Write-Log -Message "Enabling up front provisioning of pool $Name with an increment of $Increment" -Level Info
                ...
                Write-Log -Message "Enabling up front provisioning of pool $Name with $VMAmount VMs" -Level Info
                $Pool = Get-HVPool -PoolName $Name -HvServer $Global:VSIHV_ConnectionServer -SuppressInfo $true
                if ($Pool.Type -eq "AUTOMATED") {
                    $desktophelper = $desktopservice.read($hvService, $pool.id)
                    $desktophelper.getdesktopsettingshelper().setEnabled($True)
            ...
            Write-Log -Message "Enabling OnDemand provisioning of pool $Name with $NumberOfSpareVMs spare VMs" -Level Info
            $Pool = Get-HVPool -PoolName $Name -HvServer $Global:VSIHV_ConnectionServer -SuppressInfo $true
            if ($Pool.Type -eq "AUTOMATED") {
                $desktophelper = $desktopservice.read($hvService, $pool.id)
                $desktophelper.getdesktopsettingshelper().setEnabled($True)
...
function WaitFor-AvailableMachines {
    ...
    if ($RDSH) {
        ...
        while ($true) {
            ...
            Write-Log -Update -Message "$AvailableCount/$ProvisioningMachinesCount/$ErrorMachineCount/$AssignedMachineCount/$VMAmount (Available/Provisioning/Errors/Assigned/Total)..." -Level Info
