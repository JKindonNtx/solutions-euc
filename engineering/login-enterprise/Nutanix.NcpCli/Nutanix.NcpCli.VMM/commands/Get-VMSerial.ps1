...
<#
    ...
    .EXAMPLE
    PS> Get-VMSerial -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password" -VMExtID "08a4c861-3c9a-43b4-a8e0-fdb065dcfe4b"
    Gets the VM with Ext ID 08a4c861-3c9a-43b4-a8e0-fdb065dcfe4b attached Serial Ports from the Prism Central Appliance.

    .LINK
    https://github.com/nutanix-enterprise/solutions-euc/blob/main/engineering/NcpCli/Help/Get-VMSerial.md
#>
    ...
    process {
        ...
        try {
            ...
            $ApiData = (Get-NutanixApiPath -NameSpace "VMM.VMSerial")
            $ApiPath = "$($ApiData)/$($VMExtID)/serial-ports"
