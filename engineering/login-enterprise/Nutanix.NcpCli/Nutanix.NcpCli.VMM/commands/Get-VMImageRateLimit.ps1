...
<#
    ...
    .EXAMPLE
    PS> Get-VMImageRateLimit -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password"
    Gets the current Image Rate Limits from the Prism Central Appliance.

    .EXAMPLE
    PS> Get-VMImageRateLimit -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password" -ImageExtID "214313412-314132-12fda324-efdsa"
    Gets the Image with Ext ID 214313412-314132-12fda324-efdsa Rate Limit from the Prism Central Appliance.

    .LINK
    https://github.com/nutanix-enterprise/solutions-euc/blob/main/engineering/NcpCli/Help/Get-VMImageRateLimit.md
    ...
    process {
        ...
        try {
            ...
            if($null -ne $ImageExtID){
                $ApiRoot = (Get-NutanixApiPath -NameSpace "VMM.VMImageRateLimit")
                $ApiPath = "$($ApiRoot)/$($ImageExtID)"
            } else {
                $ApiPath = (Get-NutanixApiPath -NameSpace "VMM.VMImageRateLimit")
            }
