...
<#
    ...
    .EXAMPLE
    PS> Get-PrismTask -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password"
    Gets the current Tasks from the Prism Central Appliance.

    .EXAMPLE
    PS> Get-PrismTask -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password" -UUID "78a1e6e7-5900-4f2a-839e-3b993e364889"
    Gets the 78a1e6e7-5900-4f2a-839e-3b993e364889 Task from the Prism Central Appliance.

    .LINK
    https://github.com/nutanix-enterprise/solutions-euc/blob/main/engineering/NcpCli/Help/Get-PrismTask.md
#>
    ...
    process {
        ...
        try {
            ...
            if($null -ne $UUID){
                $ApiRoot = (Get-NutanixApiPath -NameSpace "Prism.Task")
                $ApiPath = "$($ApiRoot)/$($UUID)"
            } else {
                $ApiPath = (Get-NutanixApiPath -NameSpace "Prism.Task")
            }
