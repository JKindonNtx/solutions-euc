...
Import-Module $ScriptRoot\modules\VSI.Target.Azure -Force
...
ForEach ($ImageToTest in $VSI_Target_ImagesToTest) {
    ...
    for ($i = 1; $i -le $VSI_Target_ImageIterations; $i++) {
        ...
        if (-not $SkipLaunchers) {
            Connect-VSIAz  -UserName $VSI_Azure_Username `
            -Password $VSI_Azure_Password `
            -SubscriptionId $VSI_Azure_SubscriptionId
Set-VSIAzLaunchers -Amount $NumberOfLaunchers `
-NamingPattern $VSI_Launchers_NamingPattern `
-Image $VSI_Launchers_Image `
-Size $VSI_Launchers_Size `
-ResourceGroup $VSI_Launchers_ResourceGroup `
-Admin $VSI_Launchers_Admin `
-Password $VSI_Launchers_Password `
-Location $VSI_Azure_Location `
-ApplianceURL $VSI_LoginEnterprise_ApplianceURL `
-Force:$Force.IsPresent
        }
        # Will only create the pool if it does not exist
        Set-VSICTXDesktopPoolAz -ParentVM $VSI_Target_ParentVM `
        ...
        -NetworkPath $VSI_Target_NetworkPath `
        -ResourceGroup $VSI_Target_ResourceGroup `
        ...
        # Get metrics for the vm's from azure
