function Set-VSIHVDesktopPool {
        ...
        Switch ($PoolType) {
                ...
                if ($ExistingPool.Type -eq "RDS") {
                    ...
                    if ($null -ne $ExistingFarm) {
                        ...
                        if ($ParentVM -eq $CurrentParentVM -and $VMSnapshot -eq $CurrentSnapshot) {
                            Write-Log "RDS farm with name F_$Name and ParentVM: $ParentVM and Snapshot: $VMSnapshot already exists, skipping creation"
                            $CreatePool = $false
                        } else {
                            Write-Log "RDSFarm with name F_$Name already exists, but with different ParentVM configuration current: $CurrentParentVM/$CurrentSnapshot requested: $ParentVM/$VMSnapshot, removing"
                    ...
                    Write-Log "Pool with name $Name already exists, but it's not an RDS type pool, removing"
                    Remove-VSIHVDesktopPool -Name $Name
                }
            }
            ...
            "InstantClone" {
                if ($ExistingPool.Type -eq "AUTOMATED") {
                    ...
                    if ($ParentVM -eq $CurrentParentVM -and $VMSnapshot -eq $CurrentSnapshot) {
                        Write-Log "Desktop pool with name $Name and ParentVM: $ParentVM and Snapshot: $VMSnapshot already exists, skipping creation"
                        $CreatePool = $false
                    } else {
                        Write-Log "Desktop pool with name $Name already exists, but with different ParentVM configuration current: $CurrentParentVM/$CurrentSnapshot requested: $ParentVM/$VMSnapshot, removing"
                        Remove-VSIHVDesktopPool -Name $Name
                    }
                } else {
                    Write-Log "Desktop pool with name $Name already exists, but it's not an InstantClone pool, removing"
                    Remove-VSIHVDesktopPool -Name $Name
                }
            }
        }
    ...
    if ($CreatePool -eq $true) {
        Switch ($PoolType) {
            "InstantClone" {
                Write-Log "Creating DesktopPool $Name with $ParentVM/$VMSnapshot as parent"
                ...
                | Out-Null
                Write-Log "Waiting for DesktopPool $Name to be ready"
                ...
                while ($PoolStatus.Operation -ne "NONE") {
                    $PoolStatus = (Get-HVPool -PoolName $Name -HvServer $Global:VSIHV_ConnectionServer -SuppressInfo $true).AutomatedDesktopData.ProvisioningStatusData.InstantCloneProvisioningStatusData
                    Write-Log -Update "Progress:  $($Poolstatus.PendingImageProgress)"
            ...
            "RDSH" {
                ...
                if ($null -eq $ExistingFarm) {
                    Write-Log "Creating RDSH Farm F_$Name with $ParentVM/$VMSnapshot as parent"
                    New-HVFarm @CommonArguments -InstantClone `
                        -FarmName "F_$Name" `
                        -FarmDisplayName "F_$Name" `
                        -Description $Description | Out-Null
                    Write-Log "Waiting for Farm F_$Name to be ready"
                    Start-Sleep -Seconds 60
                }
                ...
                while ($FarmStatus.Operation -ne "NONE") {
                    $FarmStatus = (Get-HVFarm -FarmName F_$Name -HvServer $Global:VSIHV_ConnectionServer).AutomatedFarmData.ProvisioningStatusData.InstantCloneProvisioningStatusData
                    Write-Log -Update "Progress:  $($FarmStatus.PendingImageProgress)"
                    Start-Sleep -Seconds 10
                }
                Write-Log ""
                New-HVPool -Rds `
