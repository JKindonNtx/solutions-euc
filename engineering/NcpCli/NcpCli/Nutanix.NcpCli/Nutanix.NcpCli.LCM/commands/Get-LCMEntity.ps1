...
<#
    ...
    .EXAMPLE
    PS> Get-LCMEntity -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password"
    Gets the current Entities from the Prism Central Appliance.

    .EXAMPLE
    PS> Get-LCMEntity -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password" -UUID "78a1e6e7-5900-4f2a-839e-3b993e364889"
...
#>
    ...
    process {
        ...
        try {
            ...
            if($null -ne $UUID){
                $ApiRoot = (Get-NutanixApiPath -NameSpace "LCM.Entity")
                $ApiPath = "$($ApiRoot)/$($UUID)"
            } else {
                $ApiPath = (Get-NutanixApiPath -NameSpace "LCM.Entity")
            }
