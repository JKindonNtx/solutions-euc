...
<#
    ...
    .EXAMPLE
    PS> Get-PrismUserPolicy -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password"
    Gets the current User Policy from the Prism Central Appliance.

    .EXAMPLE
    PS> Get-PrismUserPolicy -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password" -UUID "78a1e6e7-5900-4f2a-839e-3b993e364889"
    Gets the 78a1e6e7-5900-4f2a-839e-3b993e364889 User Policy from the Prism Central Appliance.

    .LINK
    https://github.com/nutanix-enterprise/solutions-euc/blob/main/engineering/NcpCli/Help/Get-PrismUserPolicy.md
#>
    ...
    process {
        ...
        try {
            ...
            if($null -ne $UUID){
                $ApiRoot = (Get-NutanixApiPath -NameSpace "Prism.UserPolicy")
                $ApiPath = "$($ApiRoot)/$($UUID)"
            } else {
                $ApiPath = (Get-NutanixApiPath -NameSpace "Prism.UserPolicy")
            }
