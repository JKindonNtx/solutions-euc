...
<#
    ...
    .EXAMPLE
    PS> Get-FlowServiceGroup -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password"
    Gets the current Flow Service Group from the Prism Central Appliance.

    .EXAMPLE
    PS> Get-FlowServiceGroup -PrismIP "10.10.10.10" -PrismUserName "admin" -PrismPassword "password" -UUID "78a1e6e7-5900-4f2a-839e-3b993e364889"
...
#>
    ...
    process {
        ...
        try {
            ...
            if($null -ne $UUID){
                $ApiRoot = (Get-NutanixApiPath -NameSpace "Flow.ServiceGroup")
                $ApiPath = "$($ApiRoot)/$($UUID)"
            } else {
                $ApiPath = (Get-NutanixApiPath -NameSpace "Flow.ServiceGroup")
            }
