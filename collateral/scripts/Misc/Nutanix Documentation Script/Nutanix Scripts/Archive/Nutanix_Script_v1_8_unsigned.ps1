...
.DESCRIPTION
	...
	Creates a Word document named after the Nutanix Cluster Configuration.
...
foreach ($rackableUnit in Get-NTNXRackableUnit  –Verbose:$false) {
...
$RemoteSetting = Get-NTNXRemoteSupportSetting –Verbose:$false;
$Params = $null    
$Params = @{       
    Hashtable = @( ## IB - The -Hashtable parameter needs an array of hashtables. Note the @( and not @{
        @{         ## IB - This is a hash table. As it's the first, it'll be the first row in the table
            Enabled = $RemoteSetting.enable;
            Detail = $RemoteSetting.tunnelDetails;
        })
...
## IB - Rather than repeatedly calling the Get-NTXRackableUnit cmdlet, we'll store the value
$SMTPSetting = Get-NTNXSmtpServer –Verbose:$false;
$Params = $null    
$Params = @{       
    Hashtable = @( ## IB - The -Hashtable parameter needs an array of hashtables. Note the @( and not @{
        @{         ## IB - This is a hash table. As it's the first, it'll be the first row in the table
...
## IB - Loop through each returned rackable unit/object
foreach ($hostconfig in Get-NTNXHost –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $hostconfigs += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($hostalert in Get-NTNXHostAlert –Verbose:$false) {
 if ($hostalert.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $hostalert += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($hostevent in Get-NTNXHostEvent –Verbose:$false) {
    ## IB - Add each enumerated object to the array
     if ($hostevent.Resolved -eq $false) {
    $hostevent += @{
...
foreach ($CritAlert in get-NTNXAlert) {
 if ($CritAlert.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $CritAlerts += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($AuthConfig in Get-NTNXAuthConfigDirectory –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $AuthConfigs += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($ClusterConfig in Get-NTNXCluster –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $ClusterConfigs += @{
        "ID" = $ClusterConfig.id;
...
## IB - Loop through each returned rackable unit/object
foreach ($ClAlert in Get-NTNXMultiClusterAlert –Verbose:$false) {
 if ($ClAlert.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $ClAlerts += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($ClEvent in Get-NTNXMultiClusterEvent –Verbose:$false) {
 if ($ClEventt.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $ClEvents += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($CmdLet in Get-NutanixCmdletsInfo –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $CmdLets += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($Container in Get-NTNXContainer –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $Containers += @{
        "Name" = $Container.name;
        "Pool ID" = $Container.storagePoolId;
        "Marked for removal" = $Container.markedForRemoval;
        "Max Capacity" = [math]::truncate($Container.maxCapacity /1GB);
        "Explicit Reserved" = [math]::truncate($Container.totalExplicitReservedCapacity /1GB);
        "Implicit Reserved" = [math]::truncate($Container.totalImplicitReservedCapacity /1GB);
        "RF" = $Container.replicationFactor;
        "OpLog RF" = $Container.oplogReplicationFactor;
        "Provisioned Capacity" = [math]::truncate($Container.provisionedCapacity /1GB);
...
$Params = @{       
    ...
    Columns = "Name","Pool ID","Marked for removal","Max Capacity","Explicit Reserved","Implicit Reserved";
...
$Params = @{       
    ...
    Hashtable = $Containers;
    Columns = "Name","RF","OpLog RF","Provisioned Capacity","Fingerprint on Write","On Disk Dedup","Compression enabled";
    Format = -235; 
    AutoFit = $wdAutoFitContent;
}
$Table = AddWordTable @Params -NoGridLines;
...
## IB - Loop through each returned rackable unit/object
foreach ($DataStore in Get-NTNXNfsDatastore –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $DataStores += @{
        ...
        "Capacity" =  [math]::truncate($DataStore.capacity /1GB);
        "Free Space" =  [math]::truncate($DataStore.freeSpace /1GB);
...
## IB - Loop through each returned rackable unit/object
foreach ($DiskConfig in Get-NTNXDisk –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $DiskConfigs += @{
        "ID" = $DiskConfig.id;
        "Mount Path" = $DiskConfig.mountPath;
        "Disk Size" =  [math]::truncate($DiskConfig.diskSize /1GB);
...
$Params = @{       
    ...
    Columns = "ID","Mount Path","Disk Size","Hostname","CVM IP","Tier Name","Removal";
    ## IB - You only need quote the values if they contain spaces.
    ## IB - You only need specify the -Headers parameter if the column names don't work, i.e. need spaces
    ## Headers = "ID","Model","Location","Serial","Positions","Nodes"; ## If the Columns
...
## IB - Loop through each returned rackable unit/object
foreach ($DiskAlert in Get-NTNXDiskAlert –Verbose:$false) {
 if ($Diskalert.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $DisAlerts += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($DiskEvent in Get-NTNXMultiClusterEvent –Verbose:$false) {
 if ($DiskEvent.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $DiskEvents += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($ClusterLicenseConfig in Get-NTNXClusterLicenseInf –Verbose:$falseo) {
    ## IB - Add each enumerated object to the array
...
## IB - Loop through each returned rackable unit/object
foreach ($LicensingConfig in Get-NTNXLicense –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $LicensingConfigs += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($LicAllowanceConfig in Get-NTNXLicenseAllowance –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $LicAllowanceConfigs += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($HealthCheck in Get-NTNXHealthCheck –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $HealthChecks += @{
        "ID" = $HealthCheck.id;
...
## IB - Loop through each returned rackable unit/object
foreach ($CritEvent in get-NTNXEvent –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $CritEvents += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($vDisk in Get-NTNXVDisk –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $vDisks += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($StoragePool in Get-NTNXStoragePool –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $StoragePools += @{
        "ID" = $StoragePool.id;
        "Name" = $StoragePool.name;
        "Capacity" =  [math]::truncate($StoragePool.capacity /1GB);
        "Reserved Capacity" =  [math]::truncate($StoragePool.reservedCapacity /1GB);
...
$Params = @{       
    ...
    Columns = "ID","Name","Capacity","Reserved Capacity","ILM Threshold";
...
## IB - Loop through each returned rackable unit/object
foreach ($SPEvent in Get-NTNXStoragePoolEvent –Verbose:$false) {
 if ($SPevent.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $SPEvents += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($ProtDomain in Get-NTNXProtectionDomain –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $ProtDomains += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($PDCGroup in Get-NTNXProtectionDomainConsistencyGroup –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $PDCGroups += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($PSnapshot in Get-NTNXProtectionDomainSnapshot –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $PSnapshots += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($PSUnProtVM in Get-NTNXUnprotectedVm –Verbose:$false) {
    ## IB - Add each enumerated object to the array
    $PSUnProtVMs += @{
        "Name" = $PSUnProtVM.vmName;
        "OS" =  $PSUnProtVM.guestOperatingSystem;
        "Memory in GB" = $PSUnProtVM.memoryCapacityInBytes / 1GB;
        "Res. Memory" = $PSUnProtVM.memoryReservedCapacityInBytes /1GB;
        ...
        "Disk Capacity" = [math]::truncate($PSUnProtVM.diskCapacityInBytes / 1GB);
    };
} # end foreach
...
$Params = @{       
    ...
    Columns = "Name","OS","Memory in GB","Res. Memory","vCPUs","Res. HZ","NICs","Disk Capacity";
    Format = -235; 
    AutoFit = $wdAutoFitContent;
}
$Table = AddWordTable @Params -NoGridLines;
...
## IB - Loop through each returned rackable unit/object
foreach ($SPAlert in Get-NTNXProtectionDomainAlert –Verbose:$false) {
 if ($SPAlert.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $SPAlerts += @{
...
## IB - Loop through each returned rackable unit/object
foreach ($PDEvent in Get-NTNXProtectionDomainEvent –Verbose:$false) {
 if ($PDEvent.Resolved -eq $false) {
    ## IB - Add each enumerated object to the array
    $PDEvents += @{
