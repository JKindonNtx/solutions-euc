	...
	Process
	{
		...
		Switch ($PSCmdlet.ParameterSetName) 
		{
			'CustomObject' 
			{
				...
				If(-not $List) 
				{
					##Write-Verbose ("$(Get-Date): `t`tBuilding table headers");
					If($Headers -ne $null) 
					{ 
						$WordRangeString += "$([String]::Join("`t", $Headers))`n"; 
					}
					Else 
					{ 
						$WordRangeString += "$([String]::Join("`t", $Columns))`n"; 
					}
				}

				## Iterate through each PSCustomObject
				##Write-Verbose ("$(Get-Date): `t`tBuilding table rows");
				ForEach($Object in $CustomObject) 
				{
					...
					$WordRangeString += "$([String]::Join("`t", $OrderedValues))`n";
				} ## end foreach
				##Write-Verbose ("$(Get-Date): `t`t`tAdded '{0}' table rows" -f ($CustomObject.Count));
			} ## end CustomObject
			...
			{ ## Hashtable
				...
				##Write-Verbose ("$(Get-Date): `t`t`tAdded '{0}' table rows" -f $Hashtable.Count);
			} ## end default
		} ## end switch

		## Create a MS Word range and set its text to our tab-delimited, concatenated string
		##Write-Verbose ("$(Get-Date): `t`tBuilding table range");
		$WordRange = $Script:Doc.Application.Selection.Range;
		$WordRange.Text = $WordRangeString;
		...
		##Write-Verbose ("$(Get-Date): `t`tConverting range to table");
		...
		If($Format -lt 0) 
		{
			##Write-Verbose ("$(Get-Date): `t`tSetting table format");
...
Function SetWordTableAlternateRowColor 
{
	...
	Process 
	{
		...
		Write-Verbose ("{0}: `t`tSetting alternate table row colors.." -f $StartDateTime);
		...
		Write-Verbose ("{0}: `t`tDone setting alternate row style color in '{1}' seconds" -f $EndDateTime, $ExecutionTime.TotalSeconds);
	}
}
...
Function UpdateDocumentProperties
{
	...
	If($MSWORD -or $PDF)
	{
		If($Script:CoverPagesExist)
		{
			...
			###Change the following line for your script###
			_SetDocumentProperty $Script:Doc.BuiltInDocumentProperties "Subject" "Sample Script Template Report"
			...
			{
				[string]$abstract = "Script Template Report"
			}
			Else
			{
				[string]$abstract = "Script Template Report for $Script:CoName"
			}
...
if($AUTHLOCS.Length -le 0) { WriteWordLine 0 0 "No Local Administration User has been configured"} else {
...
if($AUTHRADIUSS.Length -le 0) { WriteWordLine 0 0 "No RADIUS Authentication actions has been configured"} else {
    ...
    foreach ($AUTHRADIUS in $AUTHRADIUSS) {
        ...
        [System.Collections.Hashtable[]] $RADIUSCONFIG = @(
            ...
            @{ Description = "RADIUS Server Port"; Value = Get-StringProperty $AUTHRADIUS "-serverPort" "1813"; }
        ...
        if($AUTHLDAPPOLS.Length -le 0) { WriteWordLine 0 0 "No LDAP Policy has been configured"} else { 
            ...
            if ($AUTHPOLHASH.Length -gt 0) {
                ...
                $Params = @{
                    ...
                    Columns = "Name","Action","Expr";
                    Headers = "Policy Name","Policy Action","Expression";
                    AutoFit = $wdAutoFitContent
                    Format = -235; ## IB - Word constant for Light List Accent 5
                }
                ## IB - Add the table to the document, splatting the parameters
        ...
        if($AUTHRADPOLS.Length -le 0) { WriteWordLine 0 0 "No Radius Policy has been configured"} else {        
            ...
            if ($AUTHPOLRADHASH.Length -gt 0) {
                ...
                $Params = @{
                    ...
                    Columns = "Name","Action","Expr","Priority";
                    Headers = "Policy Name","Policy Action","Expression","Priority";
                    AutoFit = $wdAutoFitContent
                    Format = -235; ## IB - Word constant for Light List Accent 5
                }
                ## IB - Add the table to the document, splatting the parameters
...
WriteWordLine 0 0 " "
    ...
    if($vServerSTAs.Length -gt 0) {
        ...
        Get-StringProperty $vServerSTA "-staServer" -RemoveQuotes;
        $vServerSTAH += @{
