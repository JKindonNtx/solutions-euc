...
# Setting variables
$RegistryHyve  = 'LocalMachine'
$RegistryKeyPath = 'HKLM:\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\7516b95f-f776-4464-8c53-06167f40cc99\8EC4B3A5-6868-48c2-BE75-4F3044BE88A7\'
...
write-log -message "Grabbing Client OS-based AD Objects information"
...
$ADVMs = $VMs.vmName | where {$clients.Name -contains $_}
...
foreach ($vm in $ADVMs){
            try {
                ...
                if ($RemoteRegistry.State -eq 'Running') {
                    ...
                    $s = New-PSSession -ComputerName $vm -ThrottleLimit 16
                    Invoke-Command -Session $s -ScriptBlock {Get-Process PowerShell} -AsJob
                }
 
                if ($RemoteRegistry.StartMode -eq 'Disabled') {
                    Set-Service -Name RemoteRegistry -ComputerName $vm -StartupType Manual #-ErrorAction Stop
                    write-log -message "$VM : Remote Registry has been Enabled"
                }
 
                if ($RemoteRegistry.State -eq 'Stopped') {
                    Start-Service -InputObject (Get-Service -Name RemoteRegistry -ComputerName $vm) #-ErrorAction Stop
                    write-log -message "$vm : Remote Registry has been Started"
                }
