...
- JK Additions
    ...
    02.08.2024: Rewrote code to make this script work with the LoginDocuments-v4 dashboard (with AlignTime function).
- To Do
...
Param(
    ...
    [Parameter(Mandatory = $false)]
    [switch]$RapidRA #If switched, then only images required for RA docs are downloaded. All other document content is honored
...
function Get-Graphs {
    ...
    foreach ($Panel in $Panels) {
        ...
        $Uri = $UpdatedUri + "&from=$($Start)&to=$($EndTime)&panelId=$($Panel)&width=1600&height=800&tz=UTC"
...
Write-Host "
--------------------------------------------------------------------------------------------------------"
if ($RapidRA){ Write-Host "Warning: RapidRA Mode selected. Only processing Rapid RA Graph List" }
...
$FormattedStartYear = (Get-UriVariable -Uri $SourceUri -Var "var-StartYear").Replace('.', '\.')
$FormattedEndYear = (Get-UriVariable -Uri $SourceUri -Var "var-EndYear").Replace('.', '\.')
...
if (-not $RapidRA) {
    if ($BootInfo) {
        if ($GraphsToExcludeList -notcontains "BootInfo") {
            ...
            $endtime = 1704067200000 + $maxboottime
            Get-Graphs -Panels $Panels -EndTime $endtime -SourceUri $SourceUri -imagePath $imagePath
        } else {
