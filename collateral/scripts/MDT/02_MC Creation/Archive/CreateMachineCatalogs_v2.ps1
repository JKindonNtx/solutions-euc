...
# ----------
$adminAddress = 'CONTMXD002' #The XD Controller we're going to execute against
$xdControllers = 'CONTMXD002'
...
$storageResource = "NTNX-LAN 2" #Storage
$hostResource = "NTNXKBG5-AHV Automated 6" #Hypervisor management

# Machine catalog properties
$machineCatalogName = "W-103"
$machineCatalogDesc = ""
$domain = "contoso.local"
$orgUnit = "OU=VSI-test,OU=Computers,OU=CORP,DC=contoso,DC=local"
$namingScheme = "c###" #AD machine account naming conventions
...
If (Test-ProvSchemeNameAvailable -AdminAddress $adminAddress -ProvisioningSchemeName @($machineCatalogName))
{
  ...
  $VM = Get-ChildItem -AdminAddress $adminAddress "XDHyp:\HostingUnits\$storageResource" | Where-Object { $_.ObjectType -eq "Template" -and $_.PSChildName -like $masterImage }
  ...
  New-ProvScheme  -AdminAddress $CVADController  -CleanOnBoot -CustomProperties "<CustomProperties xmlns=`"http://schemas.citrix.com/2014/xd/machinecreation`">`r`n  <StringProperty Name=`"ContainerPath`" Value=`"/1265.storage`"/>`r`n  <StringProperty Name=`"vCPU`" Value=`"1`"/>`r`n  <StringProperty Name=`"RAM`" Value=`"2048`"/>`r`n  <StringProperty Name=`"CPUCores`" Value=`"2`"/>`r`n</CustomProperties>" -HostingUnitName $storageResource -IdentityPoolName $identPool.IdentityPoolName -InitialBatchSizeHint "4" -MasterImageVM $VMDetails.FullPath -Metadata @{"NutanixCoresPerCpu"="2";"NutanixContainerId"="1265"} -NetworkMapping @{"0"="XDHyp:\HostingUnits\KBTest\VDI-LAN.network"} -ProvisioningSchemeName $machineCatalogName  -RunAsynchronously -Scope @() -VMCpuCount $vCPUs -VMMemoryMB $vRAM 
